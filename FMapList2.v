...
Module Type Make_fun_Type (X:OrderedType) <: S.
Module Raw := Raw X.
Module E := X.
...
Section Elt.
 ...
 Parameter MapsTo_1 : forall m x y e, E.eq x y -> MapsTo x e m -> MapsTo y e m.

 Parameter mem_1 : forall m x, In x m -> mem x m = true.
 Parameter mem_2 : forall m x, mem x m = true -> In x m.

 Parameter empty_1 : Empty empty.

 Parameter is_empty_1 : forall m, Empty m -> is_empty m = true.
 Parameter is_empty_2 :  forall m, is_empty m = true -> Empty m.

 Parameter add_1 : forall m x y e, E.eq x y -> MapsTo y e (add x e m).
 Parameter add_2 : forall m x y e e', ~ E.eq x y -> MapsTo y e m -> MapsTo y e (add x e' m).
 Parameter add_3 : forall m x y e e', ~ E.eq x y -> MapsTo y e (add x e' m) -> MapsTo y e m.

 Parameter remove_1 : forall m x y, E.eq x y -> ~ In y (remove x m).
 Parameter remove_2 : forall m x y e, ~ E.eq x y -> MapsTo y e m -> MapsTo y e (remove x m).
 Parameter remove_3 : forall m x y e, MapsTo y e (remove x m) -> MapsTo y e m.

 Parameter find_1 : forall m x e, MapsTo x e m -> find x m = Some e.
 Parameter find_2 : forall m x e, find x m = Some e -> MapsTo x e m.

 Parameter elements_1 : forall m x e, MapsTo x e m -> InA eq_key_elt (x,e) (elements m).
 Parameter elements_2 : forall m x e, InA eq_key_elt (x,e) (elements m) -> MapsTo x e m.
 Parameter elements_3 : forall m, sort lt_key (elements m).
 Parameter elements_3w : forall m, NoDupA eq_key (elements m).

 Parameter cardinal_1 : forall m, cardinal m = length (elements m).

 Parameter fold_1 : forall m (A : Type) (i : A) (f : key -> elt -> A -> A),
        fold f m i = fold_left (fun a p => f (fst p) (snd p) a) (elements m) i.

 Parameter equal_1 : forall m m' cmp, Equivb cmp m m' -> equal cmp m m' = true.
 Parameter equal_2 : forall m m' cmp, equal cmp m m' = true -> Equivb cmp m m'.
...
Module Make_fun (X: OrderedType) : Make_fun_Type X.
Module Raw := Raw X.
Module E := X.
...
Module Type Sfun' := Sfun X with Definition t := t'.
...
Module Make_ord_fun (X: OrderedType)(MapS:Make_fun_Type X)(D : OrderedType).
...
Definition eq (m m' : MapS.t D.t) := eq_list m.(this) m'.(this).
...
Module Make_ord (X:OrderedType)(D:OrderedType): 
  ...
  Module Data := D.
  Module MapS := Make_fun(X).
  Include (Make_ord_fun MapS.E MapS Data).
End Make_ord.

Module Make_ord_ex.
  Declare Module E : OrderedType.
  Declare Module Data : OrderedType.
  Declare Module MapS : Make_fun_Type E.
  Include (Make_ord_fun E MapS Data).
End Make_ord_ex.